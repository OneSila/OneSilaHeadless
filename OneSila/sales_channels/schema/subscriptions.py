...
from sales_channels.models import SalesChannelImport, SalesChannel, SalesChannelIntegrationPricelist, SalesChannelView, \
...
class SalesChannelsSubscription:
    @subscription
    async def import_process(self, info: Info, pk: str) -> AsyncGenerator[SalesChannelImportType, None]:
        async for i in model_subscriber(info=info, pk=pk, model=SalesChannelImport):
            yield i
