...
class MiraklProductFeedFileFactory:
    ...
    def __init__(self, *, feed) -> None:
        ...
        self._product_context_cache: dict[int, dict] = {}
        self._category_cache: dict[str, MiraklCategory | None] = {}
        self._category_items_cache: dict[int, list[MiraklProductTypeItem]] = {}

    def run(self) -> str:
        rendered_rows = self._build_rendered_rows()
        buffer = io.StringIO()
        writer = csv.DictWriter(
            buffer,
            fieldnames=self._build_fieldnames(rendered_rows=rendered_rows),
            delimiter=",",
    ...
    def _build_rendered_rows(self) -> list[dict[str, str]]:
        rendered_rows: list[dict[str, str]] = []
        queryset = self.feed.items.select_related("remote_product", "remote_product__local_instance").all()
        for item in queryset:
            ...
            for index, row in enumerate(item.payload_data.get("rows") or []):
                ...
                rendered_rows.append(self._build_rendered_row(item=item, row=row, offer_payload=offer_payload))
        return rendered_rows

    def _build_fieldnames(self, *, rendered_rows: list[dict[str, str]]) -> list[str]:
    ...
    def _build_rendered_row(self, *, item, row: dict, offer_payload: dict) -> dict[str, str]:
        ...
        product_row.update(self._build_internal_property_columns(context=context))
    ...
    def _build_schema_columns(self, *, context: dict) -> dict[str, str]:
        ...
        row: dict[str, str] = {}
        for item in self._get_category_items(category=context["category"]):
            property_code = item.property.code
            if property_code in self.BASE_PRODUCT_COLUMNS or property_code in self.OFFER_COLUMNS:
                continue
            row[property_code] = self._resolve_remote_property_value(context=context, remote_property=item.property)
        return row

    def _build_internal_property_columns(self, *, context: dict) -> dict[str, str]:
        row: dict[str, str] = {}
        for internal_property in context["internal_properties"]:
            code = internal_property.code
            if code in self.BASE_PRODUCT_COLUMNS or code in self.OFFER_COLUMNS:
                continue
            if code in row:
                continue
            row[code] = self._resolve_internal_property_value(context=context, internal_property=internal_property)
        return row
    ...
    def _build_offer_columns(self, *, row: dict, offer_payload: dict, context: dict) -> dict[str, str]:
        ...
        leadtime = offer_payload.get("leadtime_to_ship") or self._lookup_mapped_value(context=context, code="leadtime_to_ship")
        ...
        if quantity in (None, ""):
            quantity = self._lookup_mapped_value(context=context, code="quantity")

        return {
            "sku": offer_payload.get("shop_sku") or row.get("sku") or "",
            "product-id": offer_payload.get("product_id") or row.get("ean") or row.get("sku") or "",
            "product-id-type": offer_payload.get("product_id_type") or ("EAN" if row.get("ean") else "SHOP_SKU"),
            "description": offer_payload.get("description") or row.get("description") or row.get("short_description") or "",
            "internal-description": offer_payload.get("internal_description") or row.get("name") or row.get("sku") or "",
            "price": self._stringify(offer_payload.get("price")),
            ...
            "state": offer_payload.get("state_code") or self._lookup_mapped_value(context=context, code="state") or "11",
            ...
            "discount-price": self._stringify(discount.get("price")),
    ...
    def _get_product_context(self, *, row_product_id, fallback_product_id) -> dict:
        product_id = row_product_id or fallback_product_id
        if not product_id:
            return self._empty_context()
        if product_id not in self._product_context_cache:
            self._product_context_cache[product_id] = self._build_product_context(
                product_id=product_id,
                fallback_product_id=fallback_product_id,
            )
        return self._product_context_cache[product_id]

    def _build_product_context(self, *, product_id: int, fallback_product_id: int | None) -> dict:
        ...
        internal_properties = list(
            MiraklInternalProperty.objects.filter(
                sales_channel=self.sales_channel,
                local_instance_id__in=property_ids,
            ).select_related("local_instance")
        )
        ...
        internal_options = list(
            MiraklInternalPropertyOption.objects.filter(
                sales_channel=self.sales_channel,
                internal_property__in=internal_properties,
                local_instance__isnull=False,
            ).select_related("internal_property", "local_instance")
        )
        ...
        return {
            ...
            "internal_property_options": {
                (option.internal_property_id, option.local_instance_id): option
    ...
    def _resolve_category_remote_id(self, *, product_id: int, fallback_product_id: int | None) -> str:
        ...
        return getattr(product_category, "remote_id", "") or ""
    ...
    def _get_category_items(self, *, category: MiraklCategory) -> list[MiraklProductTypeItem]:
        if category.id not in self._category_items_cache:
            self._category_items_cache[category.id] = list(
                MiraklProductTypeItem.objects.filter(category=category)
                .select_related("property", "property__local_instance")
                .order_by("id")
            )
        return self._category_items_cache[category.id]
    ...
    def _resolve_internal_property_value(self, *, context: dict, internal_property: MiraklInternalProperty) -> str:
        product_property = context["property_by_id"].get(internal_property.local_instance_id)
        if not product_property:
            return ""
        return self._serialize_property_value(
            product_property=product_property,
            remote_value_lookup=context["internal_property_options"],
            remote_object_id=internal_property.id,
        )
    ...
    def _serialize_property_value(self, *, product_property: ProductProperty, remote_value_lookup: dict, remote_object_id: int) -> str:
        ...
        if property_type == "SELECT" and product_property.value_select_id:
            ...
            if mapped_value is not None:
                return self._stringify(getattr(mapped_value, "value", None) or getattr(mapped_value, "translated_value", None) or getattr(mapped_value, "code", None))
            ...
            for select_value in product_property.value_multi_select.all():
                ...
                values.append(
                    self._stringify(
                        ...
                        or getattr(mapped_value, "translated_value", None)
