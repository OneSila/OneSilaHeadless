# Generated by Django 5.2.4 on 2025-07-25 17:10

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ("sales_channels", "0037_alter_remoteproperty_unique_together_and_more"),
    ]

    operations = [
        migrations.CreateModel(
            name="EbayCurrency",
            fields=[
                (
                    "remotecurrency_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remotecurrency",
                    ),
                ),
            ],
            options={
                "verbose_name_plural": "eBay Currencies",
            },
            bases=("sales_channels.remotecurrency",),
        ),
        migrations.CreateModel(
            name="EbayEanCode",
            fields=[
                (
                    "remoteeancode_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteeancode",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteeancode",),
        ),
        migrations.CreateModel(
            name="EbayMediaThroughProduct",
            fields=[
                (
                    "remoteimageproductassociation_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteimageproductassociation",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteimageproductassociation",),
        ),
        migrations.CreateModel(
            name="EbayOrder",
            fields=[
                (
                    "remoteorder_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteorder",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteorder",),
        ),
        migrations.CreateModel(
            name="EbayOrderItem",
            fields=[
                (
                    "remoteorderitem_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteorderitem",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteorderitem",),
        ),
        migrations.CreateModel(
            name="EbayPrice",
            fields=[
                (
                    "remoteprice_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteprice",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteprice",),
        ),
        migrations.CreateModel(
            name="EbayProduct",
            fields=[
                (
                    "remoteproduct_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteproduct",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteproduct",),
        ),
        migrations.CreateModel(
            name="EbayProductContent",
            fields=[
                (
                    "remoteproductcontent_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteproductcontent",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteproductcontent",),
        ),
        migrations.CreateModel(
            name="EbayProductProperty",
            fields=[
                (
                    "remoteproductproperty_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteproductproperty",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteproductproperty",),
        ),
        migrations.CreateModel(
            name="EbayProperty",
            fields=[
                (
                    "remoteproperty_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remoteproperty",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remoteproperty",),
        ),
        migrations.CreateModel(
            name="EbayPropertySelectValue",
            fields=[
                (
                    "remotepropertyselectvalue_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remotepropertyselectvalue",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remotepropertyselectvalue",),
        ),
        migrations.CreateModel(
            name="EbayRemoteLanguage",
            fields=[
                (
                    "remotelanguage_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.remotelanguage",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.remotelanguage",),
        ),
        migrations.CreateModel(
            name="EbaySalesChannel",
            fields=[
                (
                    "saleschannel_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.saleschannel",
                    ),
                ),
                (
                    "environment",
                    models.CharField(
                        choices=[("production", "Production"), ("sandbox", "Sandbox")],
                        default="production",
                        help_text="eBay environment to use",
                        max_length=16,
                    ),
                ),
                (
                    "remote_id",
                    models.CharField(
                        blank=True,
                        help_text="User ID for the eBay account.",
                        max_length=64,
                        null=True,
                        unique=True,
                    ),
                ),
                (
                    "refresh_token",
                    models.CharField(
                        blank=True,
                        help_text="Refresh token used to generate new access tokens.",
                        max_length=512,
                        null=True,
                    ),
                ),
                (
                    "refresh_token_expiration",
                    models.DateTimeField(
                        blank=True,
                        help_text="When the refresh token will expire.",
                        null=True,
                    ),
                ),
                (
                    "access_token",
                    models.CharField(
                        blank=True,
                        help_text="Access token for API requests.",
                        max_length=512,
                        null=True,
                    ),
                ),
                (
                    "expiration_date",
                    models.DateTimeField(
                        blank=True,
                        help_text="Expiration datetime of the access token.",
                        null=True,
                    ),
                ),
                (
                    "state",
                    models.CharField(
                        blank=True,
                        help_text="Unique state used for OAuth verification",
                        max_length=64,
                        null=True,
                        unique=True,
                    ),
                ),
                (
                    "connection_error",
                    models.TextField(
                        blank=True,
                        help_text="Stores the last OAuth connection failure traceback.",
                        null=True,
                    ),
                ),
            ],
            options={
                "verbose_name": "eBay Sales Channel",
                "verbose_name_plural": "eBay Sales Channels",
            },
            bases=("sales_channels.saleschannel",),
        ),
        migrations.CreateModel(
            name="EbaySalesChannelView",
            fields=[
                (
                    "saleschannelview_ptr",
                    models.OneToOneField(
                        auto_created=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        parent_link=True,
                        primary_key=True,
                        serialize=False,
                        to="sales_channels.saleschannelview",
                    ),
                ),
            ],
            options={
                "abstract": False,
                "base_manager_name": "objects",
            },
            bases=("sales_channels.saleschannelview",),
        ),
    ]
