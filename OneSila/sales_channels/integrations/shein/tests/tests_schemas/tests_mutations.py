...
class SheinProductMutationTest(TransactionTestCaseMixin, TransactionTestCase):
    ...
    def test_update_shein_product_force_update_runs_create_task(self):
        with patch(
            "sales_channels.integrations.shein.flows.tasks_runner.run_single_shein_product_task_flow"
        ) as run_flow:
