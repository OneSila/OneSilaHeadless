...
from llm.mcp.tags import TAG_EDIT, TAG_PRODUCTS, tool_tags
...
class ActivateProductMcpTool(BaseMcpTool):
    ...
    tags = tool_tags(TAG_EDIT, TAG_PRODUCTS)
    ...
    async def execute(
        ...
        """Activate a product by exact product ID or exact SKU. Requires either `product_id` or `sku`."""
        try:
            ...
            return self.build_result(
                summary=f"Activated product '{response_data['name']}' ({response_data['sku']}).",
