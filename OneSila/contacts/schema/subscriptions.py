...
class ContactsSubscription:
    ...
    async def supplier(self, info: Info, pk: str) -> AsyncGenerator[SupplierType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=Supplier):
            yield i

    @subscription
    async def customer(self, info: Info, pk: str) -> AsyncGenerator[CustomerType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=Customer):
            yield i

    @subscription
    async def influencer(self, info: Info, pk: str) -> AsyncGenerator[InfluencerType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=Influencer):
            yield i

    @subscription
    async def internal_company(self, info: Info, pk: str) -> AsyncGenerator[InternalCompanyType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=InternalCompany):
            yield i

    @subscription
    async def person(self, info: Info, pk: str) -> AsyncGenerator[PersonType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=Person):
            yield i

    @subscription
    async def address(self, info: Info, pk: str) -> AsyncGenerator[AddressType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=Address):
            yield i

    @subscription
    async def shipping_address(self, info: Info, pk: str) -> AsyncGenerator[ShippingAddressType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=ShippingAddress):
            yield i

    @subscription
    async def invoice_address(self, info: Info, pk: str) -> AsyncGenerator[InvoiceAddressType, None]:
        async for i in model_subscribe_publisher(info=info, pk=pk, model=InvoiceAddress):
            yield i
